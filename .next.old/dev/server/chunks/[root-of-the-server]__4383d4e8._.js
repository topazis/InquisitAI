...
function normalizeResult(x) {
    ...
    const level = isRiskLevel(risk.level) ? risk.level : "MEDIUM";
    ...
    const out = {
        ...
        findings: {
            ...
            authenticity: findings.authenticity === undefined ? undefined : asStringArray(findings.authenticity)
        },
        actionPlan: {
    ...
    if (out.risk.rationale.length === 0) out.risk.rationale = [
        "Insufficient structured rationale returned."
    ];
    if (out.findings.reasoning.length === 0) out.findings.reasoning = [
        "Insufficient structured reasoning returned."
    ];
    if (out.actionPlan.doNow.length === 0) out.actionPlan.doNow = [
        "Pause and avoid acting on urgency until verified."
    ];
    if (out.actionPlan.doNotDo.length === 0) out.actionPlan.doNotDo = [
        "Do not send money, credentials, or codes based on this message alone."
    ];
    if (out.actionPlan.verify.length === 0) out.actionPlan.verify = [
        "Verify using a known-good contact method (not replies/links provided in the message)."
    ];
    if (out.actionPlan.escalateWhen.length === 0) out.actionPlan.escalateWhen = [
        "If money/accounts are at risk, contact your bank/service provider immediately."
    ];
    return out;
}
function buildFallback(presetId, prompt) {
    const p = (prompt || "").trim();
    if (presetId === "voicemail") {
        return {
            summary: "This resembles a classic high-pressure payment scam (urgent request + gift cards + vague story). Treat as high risk until verified offline.",
            risk: {
                level: "HIGH",
                confidence: 0.92,
                rationale: [
            ...
            notes: "Fallback report shown because live investigation was unavailable. Treat this as decision support, not legal/financial advice."
        };
    }
    if (presetId === "brain10") {
        return {
            summary: "The “10% of our brain” claim is a myth. Neuroscience shows widespread brain activity across tasks and time.",
            risk: {
                level: "LOW",
                confidence: 0.93,
                rationale: [
            ...
            notes: "Fallback report shown because live investigation was unavailable."
        };
    }
    if (presetId === "simulation") {
        return {
            summary: "The simulation hypothesis is philosophically interesting but currently lacks direct scientific tests that could confirm it.",
            risk: {
                level: "LOW",
                confidence: 0.78,
                rationale: [
    ...
    // Generic fallback
    return {
        summary: "Live investigation was unavailable. This report provides a structured risk-and-actions template based on the provided input.",
        risk: {
            level: "MEDIUM",
            confidence: 0.55,
            rationale: [
        ...
        findings: {
            ...
            authenticity: p ? [
                "Authenticity assessment depends on additional data (source, channel, metadata)."
            ] : undefined
        },
        actionPlan: {
            doNow: [
...
function makeUserPrompt(presetId, prompt) {
    return [
        `Preset: ${presetId}`,
        "",
...
async function POST(req) {
    try {
        ...
        if (!apiKey) {
            ...
            return Response.json({
                result: fallback,
                error: "OPENAI_API_KEY not set — showing fallback."
            }, {
                status: 200
            });
        }
        ...
        const messages = [
            ...
            },
            {
                ...
                content: imageDataUrl ? [
                    {
                        type: "text",
                        text: makeUserPrompt(presetId, prompt || "(no text prompt provided)")
                    },
                    {
                        type: "image_url",
                        image_url: {
        ...
        if (!normalized) {
            ...
            return Response.json({
                result: fallback,
                error: "Model returned invalid JSON — showing fallback."
            }, {
                status: 200
            });
        }
        ...
        return Response.json({
            result: fallback,
            error: err?.message || "Server error — showing fallback."
        }, {
            status: 200
        });
    }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4383d4e8._.js.map